...
from .contextual import (
    AnimalDiseaseLibraryContextual,
    AnimalDiseaseDatasetControlContextual,
)
from .tracking import AnimalDiseaseGoogleTrackMetadata
from .tracking import AnimalDiseaseProjectsGoogleMetadata
...
common_context = [AnimalDiseaseLibraryContextual, AnimalDiseaseDatasetControlContextual]


class AnimalDiseaseBaseMetadata(BaseMetadata):
    initiative = "Animal Disease"
    organization = "animal-disease"
        ...
        for fname in glob(self.path + "/*.xlsx"):
            ...
            for row in rows:
                ...
                if "bioplatforms_project_code" in obj.keys():
                    ...
                    self._logger.error(
                        "No project code found for ticket {}, sample {}. ".format(
                            obj["ticket"], obj["bioplatforms_sample_id"]
                        )
                    )
                    self._logger.error(
                        "Check with PM for updated dataset control spreadsheet. Sample ignored."
                    )
        return packages

class AnimalDiseaseONTPromethionMetadata(AnimalDiseaseBaseMetadata):
    ckan_data_type = "ad-ont-promethion"
    ...
    def __init__(
        ...
        self.google_track_meta = AnimalDiseaseGoogleTrackMetadata(logger)
        self.google_project_codes_meta = AnimalDiseaseProjectsGoogleMetadata(logger)
...
class AnimalDiseaseIlluminaShortreadMetadata(AnimalDiseaseBaseMetadata):
    ...
    metadata_patterns = [r"^.*\.md5$", r"^.*_metadata.*.*\.xlsx$"]
    metadata_urls = [
        "https://downloads-qcif.bioplatforms.com/bpa/animal_disease_staging/illumina-shortread/",
    ]
    metadata_url_components = ("ticket",)
...
class AnimalDiseasePacbioHifiMetadata(AnimalDiseaseBaseMetadata):
    ...
    metadata_urls = [
        "https://downloads-qcif.bioplatforms.com/bpa/animal_disease_staging/pacbio-hifi/",
    ]
    metadata_url_components = ("ticket",)
    resource_linkage = ("bioplatforms_library_id", "flowcell_id")
    spreadsheet = {
