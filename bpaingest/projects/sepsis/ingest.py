        ...
        for fname in glob(self.path + '/*.xlsx'):
            ...
            for row in rows:
                ...
                obj.update({
                    ...
                    'notes': 'ARP Genomics Miseq Raw Data: %s %s' % (track_meta['taxon_or_organism'], track_meta['strain_or_isolate']),
        ...
        for fname in glob(self.path + '/*.xlsx'):
            ...
            for row in rows:
                ...
                obj.update({
                    ...
                    'ticket': row.ticket,
                    'facility': row.facility_code.upper(),
                    'notes': 'ARP Genomics Pacbio Raw Data: %s %s' % (track_meta['taxon_or_organism'], track_meta['strain_or_isolate']),
                    'insert_size_range': row.insert_size_range,
                    'library_construction_protocol': row.library_construction_protocol,
                    'sequencer': row.sequencer,
    ...
    def _get_packages(self):
        ...
        for bpa_id, info in bpa_id_info.items():
            ...
            obj.update({
                ...
                'facility': row.facility_code.upper(),
                'ticket': row.ticket,
                'notes': 'ARP Transcriptomics Hiseq Raw Data: %s %s' % (track_meta['taxon_or_organism'], track_meta['strain_or_isolate']),
                'sample': row.sample,
                'library_construction_protocol': row.library_construction_protocol,
        ...
        for fname in glob(self.path + '/*.xlsx'):
            ...
            for row in rows:
                ...
                obj.update({
                    ...
                    'ticket': row.ticket,
                    'facility': row.facility_code.upper(),
                    'notes': 'ARP Metabolomics GCMS Raw Data: %s %s' % (track_meta['taxon_or_organism'], track_meta['strain_or_isolate']),
        ...
        for fname in glob(self.path + '/*.xlsx'):
            ...
            for row in rows:
                ...
                obj.update({
                    ...
                    'ticket': row.ticket,
                    'facility': row.facility_code.upper(),
                    'notes': 'ARP Metabolomics LCMS Raw Data: %s %s' % (track_meta['taxon_or_organism'], track_meta['strain_or_isolate']),
                    'sample_fractionation_extract_solvent': row.sample_fractionation_extract_solvent,
        ...
        for fname in glob(self.path + '/*.xlsx'):
            ...
            for row in rows:
                ...
                obj.update({
                    ...
                    'facility': row.facility_code.upper(),
                    'archive_ingestion_date': ingest_utils.get_date_isoformat(google_track_meta.date_of_transfer_to_archive),
                    'notes': 'ARP Proteomics MS1Quantification Raw Data: %s %s' % (bpam_track_meta['taxon_or_organism'], bpam_track_meta['strain_or_isolate']),
    ...
    def get_swath_packages(self, data_type):
        ...
        for package_name, (name, package_data_type, printable_bpa_id, track_meta, submission_meta) in list(self.package_data.items()):
            ...
            obj.update({
                ...
                'notes': 'ARP Proteomics SwathMS %s Raw Data: %s %s' % (pool, track_meta['taxon_or_organism'], track_meta['strain_or_isolate']),
                'type': self.ckan_data_type,
                'private': True,
                'data_generated': True,
            })
