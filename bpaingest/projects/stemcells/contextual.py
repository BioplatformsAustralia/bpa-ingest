...
class StemcellsTranscriptomeContextual:
    ...
    def _read_metadata(self, metadata_path):
        field_spec = [
            ...
            fld("well_number", "well number", coerce=ingest_utils.int_or_comment),
...
class StemcellsSmallRNAContextual:
    ...
    def _read_metadata(self, metadata_path):
        field_spec = [
            ...
            fld("bpa_dataset_id", "data set id", coerce=ingest_utils.extract_ands_id),
            fld("sample_id", "bpa id", coerce=ingest_utils.extract_ands_id),
            fld("plate_number", "plate number", coerce=ingest_utils.int_or_comment),
            fld("well_number", "well number", coerce=ingest_utils.int_or_comment),
            fld("sample_name", "sample name"),
            fld("replicate_group_id", "replicate group id"),
            fld("omics", "omics"),
...
class StemcellsMetabolomicsContextual:
    ...
    def _read_metadata(self, metadata_path):
        field_spec = [
            ...
            fld("plate_number", "plate number", coerce=ingest_utils.int_or_comment),
            fld("well_number", "well number", coerce=ingest_utils.int_or_comment),
            fld("sample_name", "sample name", coerce=ingest_utils.int_or_comment),
            fld("replicate_group_id", "replicate group id"),
            fld("omics", "omics"),
            fld(
                "analytical_platform",
