    ...
    def _get_common_packages(self):
        ...
        for fname in glob(self.path + "/*.xlsx"):
            ...
            for row in rows:
                ...
                if (
                    hasattr(obj, "access_control_date")
                    and obj["access_control_date"] is not None
                ):
...
class AGONTPromethionMetadata(AGBaseMetadata):
    ckan_data_type = "grasslands-ont-promethion"
    ...
    metadata_patterns = [r"^.*\.md5$", r"^.*_metadata.*.*\.xlsx$"]
    metadata_urls = [
        "https://downloads-qcif.bioplatforms.com/bpa/grasslands/ont-promethion/",
    ]
    metadata_url_components = ("ticket",)
    resource_linkage = (
        "ticket",
        ...
        obj.update(
            {
                ...
                "bioplatforms_library_id": row.library_id,
                "library_id": row.library_id.split("/")[-1],
                "sample_id": row.sample_id.split("/")[-1],
            }
        )
...
class AGMetabolomicsMetadata(AGBaseMetadata):

    ckan_data_type = "grasslands-metabolomics"
    ...
    metadata_patterns = [r"^.*\.md5$", r"^.*_[Mm]etadata.*.*\.xlsx$"]
    metadata_urls = [
        "https://downloads-qcif.bioplatforms.com/bpa/grasslands/metabolomics/",
    ]
    metadata_url_components = ("ticket",)
