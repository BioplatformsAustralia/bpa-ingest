...
class AustralianMicrobiomeSampleContextualSQLite:
    ...
    field_specs = {
        "Coastal water": [
            ...
            fld(
                "tss", "tss (mg/l)", units="mg/l", coerce=ingest_utils.get_clean_number
            ),
            fld(
                "sio2",
                "sio2 (µmol/l)",
        ...
        "Pelagic_Public": [
            ...
            fld(
                "turbidity_ctd",
                "turbidity (nephelometric turbidity units) ctd",
        ...
        "Seaweed": [
            ...
            fld(
                ...
                "average host abundance (% of individuals per m2)",
                units="%",
                coerce=ingest_utils.get_clean_number,
            ),
            fld(
                "host_abundance_seaweed",
                "host abundance (mean number ind per m2 +/- se)",
                units="mean number ind per m2 +/- se",
                coerce=ingest_utils.get_clean_number,
            ),
            fld(
                "length", "length(cm)", units="cm", coerce=ingest_utils.get_clean_number
            ),
            fld(
                "fouling",
                "fouling (%)",
            ...
            fld(
                "grazing_number", "grazing_number", coerce=ingest_utils.get_clean_number
            ),
            fld(
                "grazing",
                "grazing (%)",
        ...
        "Soil": [
            ...
            fld(
                "slope_aspect",
                "slope aspect (direction or degrees; e.g., nw or 315°)",
                units="direction or degrees; e.g., nw or 315°",
                coerce=ingest_utils.get_clean_number,
            ),
            fld(
                "australian_soil_classification",
                "australian soil classification controlled vocab (6)",
            ),
            fld(
                "soil_moisture",
                "soil moisture (%)",
        ...
        "Sponge": [
            ...
            fld(
                "geo_loc",
                "geo_loc (country:subregion)",
                units="country:subregion",
                coerce=ingest_utils.get_clean_number,
            ),
            fld("sample_site", "sample site"),
            fld("coastal_id", "coastal_id"),
